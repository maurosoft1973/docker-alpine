...
DOCKER_IMAGE=maurosoft1973/alpine
...
if [ ! -f "./alpine-$ALPINE_VERSION/${ALPINE_ARCHITECTURE}/alpine-minirootfs-$ALPINE_VERSION-${ALPINE_ARCHITECTURE}.tar.gz" ]; then
    echo "The source of alpine ${ALPINE_ARCHITECTURE} ${ALPINE_VERSION} not exist"
    exit 1
fi
...
if [ "$RELEASE" == "TEST" ]; then
    ...
    docker buildx build --platform ${PLATFORM} --build-arg BUILD_DATE=${BUILD_DATE} --build-arg ALPINE_ARCHITECTURE=${ALPINE_ARCHITECTURE} --build-arg ALPINE_RELEASE=${ALPINE_RELEASE} --build-arg ALPINE_VERSION=${ALPINE_VERSION} --build-arg ALPINE_VERSION_DATE="${ALPINE_VERSION_DATE}" -t ${DOCKER_IMAGE}:test-${ALPINE_ARCHITECTURE} -f ./Dockerfile .
    ...
    echo "Build Image: ${DOCKER_IMAGE} -> $RELEASE"
    docker buildx build --platform ${PLATFORM} --build-arg BUILD_DATE=${BUILD_DATE} --build-arg ALPINE_ARCHITECTURE=${ALPINE_ARCHITECTURE} --build-arg ALPINE_RELEASE=${ALPINE_RELEASE} --build-arg ALPINE_VERSION=${ALPINE_VERSION} --build-arg ALPINE_VERSION_DATE="${ALPINE_VERSION_DATE}" -t ${DOCKER_IMAGE}:${ALPINE_VERSION}-${ALPINE_ARCHITECTURE} -f ./Dockerfile .
else
    echo "Remove image ${DOCKER_IMAGE}:${ALPINE_ARCHITECTURE}"
    docker rmi -f ${DOCKER_IMAGE}:${ALPINE_ARCHITECTURE} > /dev/null 2>&1

    echo "Build Image: ${DOCKER_IMAGE} -> $RELEASE"
    docker buildx build --platform ${PLATFORM} --build-arg BUILD_DATE=${BUILD_DATE} --build-arg ALPINE_ARCHITECTURE=${ALPINE_ARCHITECTURE} --build-arg ALPINE_RELEASE=${ALPINE_RELEASE} --build-arg ALPINE_VERSION=${ALPINE_VERSION} --build-arg ALPINE_VERSION_DATE="${ALPINE_VERSION_DATE}" -t ${DOCKER_IMAGE}:${ALPINE_ARCHITECTURE} -f ./Dockerfile .
fi
